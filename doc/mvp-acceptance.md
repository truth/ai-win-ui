...
## 多监视器与跨屏 DPI 切换验证 (Wave3 · H8)

当系统连接了多个具有不同 DPI/缩放比例的显示器时，验证 UI 缩放与渲染器是否能正确响应：

### 验收步骤
1. **多窗跨屏移动验证**：
   - 运行任意 Demo（例如 `layouts/core_validation.xml`）。
   - 将应用窗口从一个缩放比例为 100% 的显示器，拖拽到另一个缩放比例为 125% 或 150% 的显示器中。
   - **预期结果**：当窗口的大部分进入新显示器时，窗口应该瞬间自适应刷新缩放大小，文本、卡片、图片比例应该同步放大/缩小，没有任何模糊、布局折叠、或按钮错位。
2. **多监视器居中启动验证**：
   - 将光标放置在第二个显示器（非主显示器）的空白处。
   - 运行程序，确认启动的窗口在光标所在的当前显示器居中，且启动时的 DPI 缩放大小正确对应当前显示器，而不是始终使用主显示器的 DPI 启动。
3. **分辨率/拓扑变更验证**：
   - 在程序运行过程中，在 Windows 系统的“显示设置”中，更改当前显示器的分辨率或缩放比例。
   - **预期结果**：程序应捕获到 `WM_DPICHANGED` 或 `WM_DISPLAYCHANGE` 并自动重新计算布局并重绘，界面保持清晰，组件排列符合新的缩放比例。
