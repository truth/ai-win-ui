...
## Theme vs Style Catalog 职责边界（S2）

两者都在启动 `BuildUI` 时加载，但**解决不同问题**，不要混用文件格式或语义。

```
布局节点
  ├─ quick-set / 内联属性     background="#.." fontSize="14"
  ├─ $style.name  ──────────► StyleCatalog → ComponentStyle（多状态外观包）
  └─ 属性值里的 $color.x ───► Theme → Color / spacing 标量
         $spacing.lg
```

| 维度 | **Theme** (`themes/*.json`) | **Style catalog** (`styles/*.json`) |
|------|-----------------------------|-------------------------------------|
| **问题** | 设计 token：语义色、间距刻度 | 可复用控件外观：按钮/卡片/列表壳 |
| **语法** | `$color.*` `$spacing.*` `$radius.*` `$fontSize.*` `$borderWidth.*` | `$style.name` / `styleRef` / `extend` |
| **粒度** | 单个颜色或数字 | 整套 `base/hover/pressed/...` + 嵌套 item/tab/dropdown |
| **谁写** | 设计系统 / 换肤（S4 目标） | 组件库 / demo 共享皮肤 |
| **运行时** | `UIContext::theme`；控件 `DefaultStyle(theme)` | `UIContext::styleCatalog`；parser 解析时解析 |
| **Env** | `AI_WIN_UI_THEME=themes/dark.json`（默认 `themes/default.json`） | 自动 `styles/default.json` + 可选 `AI_WIN_UI_STYLES=` 合并 |
| **缺文件** | 跳过；无 theme 时 `$color` 解析失败回退 | 无 catalog 时 `$style` 静默跳过 |
| **换肤方向** | 换 JSON → 标量全树重解析（S4） | 换/合并 catalog → 需重新 `BuildUI` 或树级 ApplyStyle |

### 该用谁？

| 场景 | 推荐 |
|------|------|
| 全应用主色、表面色、字号刻度 | Theme token |
| 「主按钮长什么样」（含 hover） | Catalog `$style.primaryButton` |
| 单页一次性背景 | 布局 quick-set `background=` |
| Catalog 里按钮背景想跟主题走 | styles JSON 写 `"background": "$color.accent"` |
| 运行时 light/dark 切换（S4） | 优先换 Theme；catalog 字段尽量引用 token 而非硬编码 hex |

### 调试：启动加载路径（S3）

Debug 输出（`OutputDebugString`，用 DebugView / VS 输出窗）：

```
[Theme] loaded=themes/default.json colors=N spacings=M
[Styles] files=styles/default.json styleCount=K
[Env] ... themeFile=themes/default.json stylesFiles=styles/default.json
```

粘连 env 时若看到非预期 `themeFile` / `stylesFiles`，执行 `scripts/clear_ai_win_env.ps1` 或设 `AI_WIN_UI_IGNORE_ENV=1`。
